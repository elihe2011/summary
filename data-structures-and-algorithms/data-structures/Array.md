...
## 8.1 [煎饼排序](https://leetcode-cn.com/problems/pancake-sorting/)
...
```
1. 找到数组中最大的数
2. 从头到该最大数位置翻转数组，使该最大数位于数组头
3. 再次反转整个数组，使该最大数位于数组尾
4. 数组长度减1，递归上述步骤，直至子数组长度为1
```

```go
func pancakeSort(arr []int) []int {
	var res []int
	sort(arr, len(arr), &res)
...
func sort(cakes []int, n int, res *[]int) {
	...
	// 第一次反转：将最大的 Cake 放最上面
	reverse(cakes, 0, maxCakeIndex)
	*res = append(*res, maxCakeIndex+1)

	// 第二次反转：将最大的 Cake 放最下面
	reverse(cakes, 0, n-1)
	*res = append(*res, n)
