...
有效的格式：整数部分必选，小数部分、指数、或类型后缀是可选的，但至少需要一个
...
Rust 的字符类型 char 代表一个单独的 Unicode 字符，是一个 32 位的值。字符字面量被单引号包围：`'*'` 和 `'\x2A'`

Rust 使用 char 表示单个字符，但是也 UTF-8 编码字符串和文本流。因此，**String 表示的文本时一个 UTF-8 字节序列，而不是字符的数组**。
...
元组是一种可以组合不同类型值得数据结构，长度固定。可作为函数返回值来返回多个值；

字面量 `("lonely hearts",)` 是一个包含单个字符串的元组，类型是 `(&str,)`。最后的逗号是必须的，为了和单纯地用括号把表达式括起来相区分。
...
**类型 `[T; N]` 表示一个有 N 个 T 类型元素的数组，每个数组的长度必须在编译期已知，不能添加新元素或缩减元素。**
...
```rust
fn main() {
    let a: [String; 8] = std::array::from_fn(|_| String::from("just a test"));
    println!("{:?}", a);
}
```
...
**切片 [T]**，没有长度。是对数组或 vector 的部分连续引用，它使得可以高效地访问序列的子部分而不需要复制。切片对于函数参数非常有用，因为它们允许函数处理数组或向量的任何部分。

因为切片可以是任意长度，因此切片不能直接存储在变量中或者作为参数传递。切片必须通过引用传递。



**切片的引用是胖指针**：包含指向切片中第一元素的指针和切片中元素数据的双字值。
