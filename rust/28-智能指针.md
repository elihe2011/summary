...
**堆内存则是从地位地址向上增长，堆内存通常只受物理内存限制，而且通常不是连续的**，因此从性能角度看，栈比堆更高。
...
而堆上则不然，底层数据并不会被拷贝，转移所有权仅仅时复制一份栈中的指针，再将新的指针赋予新的变量，然后让拥有旧指针的变量失效，最终完成所有权转移。
...
Box::leak 可以消费掉 Box 并强制目录值从内存中泄漏。其主要用于在运行期初始化值，且该值是全局有效，即和整个程序活得一样久。例如一个存储配置的结构体实例，它在运行期动态插入内容，那么就可以将其转为全局有效，虽然 `Rc/Arc` 也可以实现此功能，但是 `Box::leak` 是性能最高的。
...
自定义智能指针，需要实现 `Deref` trait，以支持 `*` 解引用：
...
总结：**一个类型为 `T` 的对象 `foo`，如果 `T: Deref<Target=U>`，那么 `foo` 的引用 `&foo` 在应用的时候会自动转换为 `&U` **
...
引用计数(reference counting)，通过记录一个数据被引用的次数来确定该数据是否正在被使用。但引用计数归零时，代表该数据不再被使用，可以被释放。
...
Rust 提供 `Cell` 和 `RefCell` 用于内部可变性，支持在拥有不可变引用的同时修改目标数据。
