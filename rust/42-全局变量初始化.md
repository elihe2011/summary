...
`std::sync::LazyLock` 和 `lazy_static` 或 `once_cell::sync` 中的 `Lazy` 都是 Rust 中用于延迟初始化的工具。
...
```rust
static EXPENSIVE_RESULT: LazyLock<Vec<u32>> = LazyLock::new(|| {
    // 这个计算只会执行一次
...
```rust
static PROGRAM_START: LazyLock<SystemTime> = LazyLock::new(|| {
    SystemTime::now()  // 在第一次访问时才获取时间
});
```
...
static DB_CONNECTION: LazyLock<Mutex<Connection>> = LazyLock::new(|| {
    Mutex::new(create_database_connection())
});
```
...
**`std::sync::LazyLock` (Rust 1.70+)**

```rust
use std::sync::LazyLock;

// 需要明确的类型注解
static VALUE: LazyLock<i32> = LazyLock::new(|| 42);
```
...
| 特性     | `std::sync::LazyLock` | `lazy_static::Lazy` |
| :------- | :-------------------- | :------------------ |
| 标准库   | ✅ 是                  | ❌ 第三方            |
| 语法     | 使用构造器            | 使用宏              |
| 类型推断 | 需要类型注解          | 自动类型推断        |
...
```rust
// 1. 配置文件加载
static CONFIG: LazyLock<Config> = LazyLock::new(|| Config::from_file("config.toml"));
...
// 4. 缓存计算结果
static CACHE: LazyLock<HashMap<String, Value>> = LazyLock::new(|| 
...
`OnceLock` 和 `OnceCell` 是 Rust 中用于**一次性初始化**的工具，它们提供了比 `LazyLock` 更灵活的单次初始化能力。

### 7.2.1 **核心概念**

**`std::sync::OnceLock` (Rust 1.70+ 标准库)**
...
**`once_cell::sync::OnceCell` (第三方库，标准库前身)**
...
fn get_data() -> &'static [i32] {
    GLOBAL_DATA.get_or_init(|| init_data())
}

// 或者手动初始化
fn initialize() {
    let _ = GLOBAL_DATA.set(vec![10, 20, 30]);
}
```
...
| 方法                | `OnceLock` | `OnceCell` | 说明                            |
| :------------------ | :--------- | :--------- | :------------------------------ |
| `new()`             | ✅          | ✅          | 创建未初始化的实例              |
| `get()`             | ✅          | ✅          | 获取引用（如果已初始化）        |
| `get_mut()`         | ✅          | ✅          | 获取可变引用（需要 `mut`）      |
| `set()`             | ✅          | ✅          | 尝试设置值，失败返回错误        |
| `get_or_init()`     | ✅          | ✅          | 获取或初始化                    |
| `get_or_try_init()` | ✅          | ✅          | 获取或尝试初始化                |
| `into_inner()`      | ✅          | ✅          | 消耗并返回值                    |
| `take()`            | ❌          | ✅          | 取出值并置空（`OnceCell` 特有） |



### 7.2.4 **关键特性**

- **线程安全**：两者都是线程安全的，但初始化逻辑只执行一次。
...
fn main() {
    // 第一次访问时初始化
    let name = USER_NAME.get_or_init(|| {
        println!("Initializing user name...");
        "Alice".to_string()
    });
    
    // 后续访问直接获取
    let name2 = USER_NAME.get().unwrap(); // 不会再次初始化
}
```
...
static DB_CONNECTION: OnceLock<Database> = OnceLock::new();

fn initialize_database() -> Result<(), Box<dyn Error>> {
    let conn = Database::connect("localhost:5432")?;
...
### 7.2.5 **与 `LazyLock` 的区别**

| 特性           | `OnceLock`/`OnceCell`     | `LazyLock`         |
| :------------- | :------------------------ | :----------------- |
| **初始化时机** | 显式控制                  | 首次访问时自动     |
| **初始化失败** | 可以处理（返回 `Result`） | 直接 panic         |
| **灵活性**     | 更高                      | 较低               |
| **使用复杂度** | 稍高                      | 简单               |
| **默认值**     | 没有默认初始化函数        | 需要提供初始化函数 |
...
// 使用 get_or_try_init 处理错误
fn ensure_logger() -> &'static Logger {
    LOGGER.get_or_try_init(|| {
        Logger::new("app.log").map_err(|e| format!("Failed: {}", e))
...
fn reload_cache() {
    let new_cache = Cache::reload_from_disk();
    let old_cache = CACHE.take(); // 取出旧值
    CACHE.set(new_cache).unwrap();
    // 可以清理 old_cache
}
```
...
struct App {
    ...
    cache: OnceLock<HashMap<String, Value>>,
}
...
impl App {
    ...
    fn init_config(&self, path: &str) {
        self.config.get_or_init(|| Config::load(path));
    }
}
```
...
**使用 `OnceLock`/`OnceCell` 当：**

1. 需要手动控制初始化时机
2. 初始化可能失败，需要错误处理
3. 可能需要重新初始化（`OnceCell::take()`）
4. 初始化逻辑复杂，需要条件判断
...
```rust
// OnceLock - 适合多次读取，一次写入
static DATA: OnceLock<Vec<u8>> = OnceLock::new();

// 初始化（一次）
DATA.get_or_init(|| expensive_computation());

// 多次读取（高效）
for _ in 0..1000 {
    let data = DATA.get().unwrap(); // 只是读取引用
}
```
...
static ASYNC_DATA: OnceCell<String> = OnceCell::new();

async fn get_async_data() -> &'static str {
    ASYNC_DATA.get_or_init(|| async {
...
- **`OnceLock`**：标准库版本，推荐在新项目使用
- **`OnceCell`**：功能更丰富，有 `take()` 等方法
- **`LazyLock`**：更简单的自动延迟初始化
- **选择**：需要错误处理或手动控制 → `OnceLock`；简单延迟初始化 → `LazyLock`
