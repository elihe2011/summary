...
crate 是 Rust 程序的最小单元。如下是一个简单的 crate。**crate root** 是编译器构建程序的起点。
...
- **binary crate**：独立的可执行文件，包含一个 main 函数，可直接运行。
- **library crate**：一组功能的集合，供其它 crate 使用，没有 main 函数，无法独立运行。
...
**一个包可以包含<font color="red"> 多个 Binary crate，但只能包含一个 Library crate</font>**
...
在 Rust 中，crate 是一个编译单元，是 Rust 编译器一次性考虑的最小代码量。当项目逐渐庞大，难以维护或 cargo 打包时间过长，可以将其拆分多个更小、更易于管理的组件。

工作区 Workspace 是一种在同一 `Cargo.toml` 根目录下管理多个 crate 的机制。它允许将多个相关的 crate 结合起来，协同工作、共享依赖关系，以及更方便地进行管理和构建。这些 crate 共享同一套依赖缓存、Cargo.lock 和 target 输出目录。



### 11.3.1 优势

#### 11.3.1.1 统一依赖管理，避免版本地狱
...
子 crate 的 Cargo.toml 直接引用：

```toml
[dependencies]
serde = { workspace = true }
tpkio = { workspace = true }
```
...
cargo 为了换取极高性能和极高安全性运行时，在编译时存在编译中间产物过大、编译速度过慢的问题。

将一个中大型的 crate，分拆成多个小的 crate，cargo 可以识别：
...
在 workspace 中，可以将公共逻辑抽到独立的 crate (如 core 或 utils)，然后各业务模块 (如 service、cli、web等) 中引用这些 crate
...
#### 11.3.1.4 共享构建产物 (target目录)

所有 crate 默认使用 同一个 target 目录，这样可以避免重复编译相同依赖，节省磁盘空间和编译时间。

例如独立编译 api 和 cli 时，不会重复构建 common 依赖 



#### 11.3.1.5 支持多二进制或多库统一管理

一个 workspace 可以包含多个可执行程序，比如：
...
编辑 my_lib/src/lib.rs：

```rust
pub fn greet(name: &str) -> String {
    format!("Hello, {}! Welcome to workspace from my library!", name)
}
...
mod tests {
    ...
    #[test]
    fn it_works() {
        let result = "Hello, Rust! Welcome to workspace from my library!".to_string();
        assert_eq!(result, greet("Rust"));
    }
}
```
...
注意：**修改了本地库，需要在主项目中执行 `cargo update` 来更新**
