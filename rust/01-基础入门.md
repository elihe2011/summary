...
```bash
rustc --version      # 查看当前 rustc 版本
rustup show          # 查看当前 toolchain、active override 等
```
...
```bash
rustup install 1.88.0       # 安装新版本

rustup default 1.88.0       # 全局切换到 1.88.0

rustup override set 1.88.0  # 或者只在当前项目目录使用
```
...
```bash
# 如果已安装需要更新，可先 remove 再 add，或直接 rustup update 会同步组件
...
如果有 C/C++ 原生依赖（build.rs、bindgen 等），可能需要重新编译或更新对应的依赖。
...
- 本地项目固定（推荐）：在项目根放 `rust-toolchain.toml`：
...
## 5.7 回退 / 删除旧 toolchain

```
rustup toolchain uninstall 1.87.0
```
...
- **编译错误或依赖报错**：先 `cargo update`，检查依赖是否需要升级（部分 crate 可能对编译器版本有假定）。
- **Clippy/rustfmt 组件找不到**：执行 `rustup component add clippy rustfmt`，并确认 toolchain 与当前 active 匹配。
- **工具链没有切换生效**：确认没有目录级 `rustup override` 或项目的 `rust-toolchain(.toml)` 覆盖；使用 `rustup show active-toolchain` 查看当前生效的。
- **Windows 用户**：如果不是通过 rustup 安装（例如 MSYS2/系统包管理器），优先使用 rustup 安装/管理以避免版本混淆。
