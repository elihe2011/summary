...
可拷贝的类型：**任何基本类型的组合可以 Copy (不需要分配内存或某种形式资源)**
...
借用检查器在编译器执行程序之前可以防止悬空引用、use-after-free 错误和编译时的数据竞争。但是，在分配内存时，如果没有在整个执行过程中删除内存，则可能发生内存泄漏。
...
```rust
fn main() {
    // 一个 String 类型的值，内存分配在堆上。
    // s1 是这块内存的所有者。
    let s1 = String::from("hello"); 

    // 将 s1 赋值给 s2 时，发生了“所有权转移”（Move）。
...
    let x: i32 = 5;
    ...
    // 直接在栈上复制一份数据，未发送所有权转移
    let y: i32 = x;

    println!("x = {}, y = {}", x, y);  // 没任何问题
}
```

如下情况，**在赋值时自动复制，而不是转移所有权，也被称为实现了 `Copy trait`：**

- 完全存储在栈上
- 大小固定的简单类型，如整数、浮点数、布尔值、字符等

**栈 (Stack) 和 堆 (Heap)**：

- **栈**：<font color="red">存储大小已知、固定不变的数据</font>。特点是“后进先出”，数据存取速度非常快，因为操作系统不需要寻找空间，总在栈顶进行操作。函数调用时的参数、局部变量通常存储在这里。

* **堆**：<font color="red">存储大小未知或可能变化的数据</font>。访问堆上的数据比访问栈要慢，因为需要通过指针间接访问，内存容易碎片化，所以 Rust 的所有权系统主要为了更好地管理堆内存。
...
    {
        // 内部作用域开始
        let s: String = String::from("hello"); // s 是 "hello" 这个 String 值的所有者

        println!("s 的值是: {}", s); // 可以正常使用 s
    } // 内部作用域结束
    // "hello" 的所有者 s 离开了作用域，Rust 自动调用 drop 函数，释放 s 所拥有的 String 内存。
...
如果需要 **深拷贝** 堆上的数据，而不是移动所有权，可以使用 `clone()`。默认情况下，Rust 不会自动创建数据的深拷贝，相较于所有权移动，clone 更耗资源。
...
## 6.6 借用 (Borrowing) 和 引用 (References)

**借用**允许在不转移所有权的情况下，创建一个指向值的 **引用**，它的性能损耗比所有权转移低，类似 C/C++ 的指针。

借用分为两类：

- 不可变借用 (Immutable Borrow)：`&T`，只读，不能修改
- 可变借用 (Mutable Borrow)： `&mut T`，读和写

借用规则：

1. 在同一个作用域内，一个值可以有任意多个不可变借用
2. 但只能有一个可变借用
3. 不可变借用和可变借用不能同时存在
...
fn main() {
    ...
    
    {
        ...
        let r2 = &s;  // ok, 创建第二个不可变引用
    
        // let r3 = &mut s; // failed, 不能在不可变引用存在的情况下创建可变引用
        // error[E0502]: cannot borrow `s` as mutable because it is also borrowed as immutable
    
        println!("r1: {}, r2: {}", r1, r2);
    }
    
    let r3 = &mut s;  // ok, 不可变引用 r1 & r2 作用域结束，被销毁了，可以创建可变引用了
    println!("r3: {}", r3);
    
    *r3 = "world".to_string(); // ok，通过可变引用修改
    println!("s: {}", s);
}
```
